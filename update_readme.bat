...
if not exist "%README_FILE%" (
    ...
    echo - **Docker Compose** doit être installé. Pour l'installer, suivez les instructions ici : [Installation de Docker Compose](https://docs.docker.com/compose/install/) , télécharger docker compose ici https://github.com/docker/compose/releases/tag/v2.34.0 >> %README_FILE%
    echo. >> %README_FILE%
    ...
    echo ```bash >> %README_FILE%
    echo cd backend ensuite tapez la commande docker build -f frontend/Dockerfile -t tickets-frontend ./frontend >> %README_FILE%
    echo cd frontend ensuite tapez la commande docker build -f backend/Dockerfile -t tickets-backend ./backend >> %README_FILE%
    echo ``` >> %README_FILE%
    echo. >> %README_FILE%
    echo 3. Démarrez les conteneurs avec Docker Compose : >> %README_FILE%
    echo ```bash >> %README_FILE%
    echo cd infrastructure ensuite tapez la commande docker-compose -f infrastructure/docker-compose.yml up -d  ou exécuter les services depuis le fichier docker-compose.yml >> %README_FILE%
    echo ``` >> %README_FILE%
    ...
    echo │   │   ├── Config >> %README_FILE%
    echo │   │   ├── Controllers >> %README_FILE%
    echo │   │   ├── Models >> %README_FILE%
    echo │   │   ├── Routes >> %README_FILE%
    echo │   │   ├── Routes >> %README_FILE%
    ...
    echo │   │   ├── components/ >> %README_FILE%
    echo │   │   ├── App.js >> %README_FILE%
    echo │   │   ├── index.js >> %README_FILE%
    echo │   ├── package.json >> %README_FILE%
    echo │   └── ... >> %README_FILE%
    ...
    echo docker build -f backend/ticket-service/Dockerfile -t tickets-backend ../backend >> %README_FILE%
    echo ``` >> %README_FILE%
    echo. >> %README_FILE%
    echo 3. Démarrez les conteneurs avec Docker Compose : >> %README_FILE%
    echo ```bash >> %README_FILE%
